...
void AnimationBonesSandboxSample::init()
{
	...
	// Enable skeleton visualization for all bones
	skinned->setVisualizeAllBones(true);

	// Create a sample LookAt chain from spine to head.
	// Weights increase from spine (0.05) to head (1.0) for natural look distribution.
	{
	...
	// Create a sample IK chain for the left leg (thigh -> shin -> foot)
	{
	...
	// Create a sample rotation constraint for the head bone (limits head rotation range)
	{
	...
	// Initialize and add editor panels
	...
	// Create a separate window for the editors
