// Demonstrates FileSystem mount points for accessing external folders and packages.
// Mount points expose external directories or .zip/.ung packages as virtual paths,
// enabling runtime loading of assets from locations outside the project data folder.
...
// Virtual paths are configured and UI is initialized.
...
// Sample UI with mount controls and status display is created.
...
// Mount/unmount buttons and image preview sprite are created for a single external resource.
void MountPointsSample::create_mount_widget(WidgetSpritePtr &sprite, const char *external_name, const char *type)
{
	// Absolute source path and target virtual mount path are resolved
...
// Status label text and color are updated.
...
// Mount point info is displayed in the label at specified index.
...
// Both folder and package mount statuses are refreshed in the UI.
...
// Preview images are loaded from mounted paths or fallback textures are used.
void MountPointsSample::update_images()
{
	// Folder image is loaded if mount exists, otherwise fallback is used
	...
	// Package image is loaded if mount exists, otherwise fallback is used
