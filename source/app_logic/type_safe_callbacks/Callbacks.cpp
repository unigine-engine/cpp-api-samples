// Demonstrates MakeCallback() for creating type-safe function wrappers. Shows binding
// free functions vs member methods, pre-binding arguments at creation time, and
// overriding bound arguments at runtime via run() with different argument counts.
...
// Free functions in anonymous namespace - demonstrate callback wrapping
// for standalone functions (as opposed to member methods)
namespace
{
...
void Callbacks::init()
{
	...
	// MakeCallback() binds arguments at creation time.
	// For member methods, first argument is the object instance.
	// Arguments can be overridden when calling run() - runtime args take precedence.
	...
	// Demonstrate argument override: run() with args replaces bound args from left
	Log::message("\n");
	callback_func_0->run();
