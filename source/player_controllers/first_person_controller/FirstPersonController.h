// Full-featured first-person character controller using capsule collision.
// Supports walking, running, crouching, jumping, auto-stepping on stairs,
// slope handling, and physics object interaction. Uses shape-surface
// collisions for ground detection and custom collision resolution.
...
// First-person character controller with capsule physics, movement modes,
// and configurable input bindings for keyboard and gamepad.
class FirstPersonController : public Unigine::ComponentBase
{

	// Comma-separated list of key names for PROP_PARAM Switch dropdowns
...
#pragma region EditorParameters
	...
	// Which analog stick to use for movement or camera
	...
	// Camera creation strategy
	enum CameraMode
	{
		NONE = 0,				// No camera management
		CREATE_AUTOMATICALLY,	// Controller creates its own PlayerDummy
		USE_EXTERNAL			// Use camera assigned in editor
	};
...
private:
	void init();			// Creates physics body, camera, precomputes angles
	void update();			// Main loop: input, physics sub-steps, crouch, camera
	void update_physics();	// Fixed-rate physics: applies impulses to rigid bodies

	void update_move_directions(float ifps);	// Reads keyboard/gamepad input
	void check_move_and_stair();				// Raycasts for walkable surfaces and stairs
	void update_velocity(float ifps, float update_part);	// Applies acceleration, damping, gravity
	void update_collisions(float ifps);			// Resolves capsule contacts with world
	void try_move_up(float ifps);				// Auto-step: elevate over obstacle
	void move_side(float ifps);					// Horizontal collision pass
	void try_move_down(float ifps);				// Auto-step: drop back to ground
	void update_crouch(float ifps);				// Smooth crouch transition state machine
	void update_camera();						// Positions camera with offsets and rotation
	void swap_interpolation_direction(Unigine::Math::Scalar start_height, Unigine::Math::Scalar end_height);	// Reverses crouch lerp
	void update_player_height(Unigine::Math::Scalar height);	// Resizes capsule and camera offset
...
private:
	...
	bool is_initialized = false;	// True after successful init()

	Unigine::Math::vec3 additional_camera_offset = Unigine::Math::vec3_zero;	// Extra offset applied to camera
	Unigine::Math::quat additional_camera_rotation = Unigine::Math::quat_identity;	// Extra rotation for camera

	bool is_ground = false;			// Standing on walkable surface
	bool is_ceiling = false;		// Head touching ceiling
	bool is_crouch = false;			// Currently crouching
	bool is_horizontal_frozen = false;	// Velocity below physics threshold

	// Slope basis vectors for movement aligned to surface
	...
	float vertical_velocity = 0.0f;	// Vertical speed (gravity, jump)

	#pragma endregion

	// Movement thresholds and offsets
	const Unigine::Math::vec2 forward { 0, 1 };	// Forward input direction
	const Unigine::Math::vec2 right { 1, 0 };	// Right input direction
	const float skin_width_offset = 0.05f;		// Collision skin for overlap prevention
	const float auto_stepping_speed_threshold = 0.1f;	// Min speed to trigger auto-step
	const float large_epsilon = 0.001f;			// Tolerance for slope comparisons

	float player_ifps = 1.0f;	// Inverse of player update rate

	Unigine::InputGamePadPtr game_pad;	// Connected gamepad (if any)

	Unigine::BodyDummyPtr body;				// Physics body for collision
	Unigine::ShapeCapsulePtr shape;			// Player collision capsule
	Unigine::ShapeCylinderPtr interaction_shape;	// Slightly larger shape for pushing objects

	Unigine::Vector<Unigine::ShapeContactPtr> contacts;	// Collision contacts buffer
	bool is_available_side_move = false;	// Raycast found walkable surface ahead
	bool is_available_stair = false;		// Raycast found stair surface ahead
	bool has_bottom_contacts = false;		// Any contacts on bottom hemisphere
	bool is_heavy_contacts = false;			// Contact count exceeds threshold

	float camera_vertical_angle = 0.0f;		// Pitch angle (look up/down)
	float camera_horizontal_angle = 0.0f;	// Yaw angle (look left/right)
	Unigine::Math::vec3 camera_crouch_offset = Unigine::Math::vec3_zero;	// Camera height adjustment when crouching

	Unigine::Math::vec2 horizontal_move_direction = Unigine::Math::vec2_zero;	// Input direction from keys/stick
	float vertical_move_direction = 0.0f;	// Jump impulse this frame

	float max_air_speed = 0.0f;	// Speed limit while airborne

	bool used_auto_stepping = false;		// Auto-step was applied this frame
	Unigine::Math::Scalar last_step_height = 0.0f;	// Height of last auto-step

	// Precomputed cosines of angle thresholds
	...
	Unigine::Math::Mat4 world_transform = Unigine::Math::Mat4_identity;	// Current player transform

	Unigine::WorldIntersectionNormalPtr normal_intersection;	// Raycast result with normal

	// Crouch state machine phases
	enum CrouchPhase
	{
		STAND = 0,		// Full height
		MOVE_DOWN,		// Transitioning to crouch
		CROUCH,			// Crouched
		MOVE_UP			// Transitioning to stand
	};

	// Interpolation state for smooth crouch transitions
	struct CrouchState
	{
		CrouchPhase phase = CrouchPhase::STAND;
		float current_time = 0.0f;				// Time remaining in transition
		Unigine::Math::Scalar current_height;	// Current capsule height
		Unigine::Math::Scalar start_height;		// Height at transition start
		Unigine::Math::Scalar end_height;		// Target height
	};

	CrouchState crouch_state;	// Current crouch transition state
