// Generates grids of procedural mesh objects using async callbacks. Compares three
// procedural modes: Dynamic (constant memory), File (disk-backed), and Blob (RAM-cached).
// Memory statistics show RAM/VRAM usage differences between modes during runtime.
...
// Constant to convert bytes to MB
constexpr float B2MB = 1.f / (1024 * 1024);

void ProceduralMeshGenerator::init()
{
	// Calculate object count for the current grid size
...
{
	// Check if all objects have finished generating
	...
	// Check if deletion of all objects has finished
	if (is_deleting_done)
	{
	...
	// Apply a new field size when no generation is in progress
...
void ProceduralMeshGenerator::create_objects()
{
	...
	// If objects already exist, clear them first; creation will resume after deletion completes
	...
	// Pre-place objects in a grid with spacing
	...
	for (int x = 0; x < size; x++)
	{
		for (int y = 0; y < size; y++)
		{
			...
			// This call will:
			// 1) Build a mesh via create_mesh callback,
			// 2) Create/load MeshRender based on current_mesh_render_flag,
			// 3) Apply the mesh to the object,
			// 4) Call create_done when finished.
...
void ProceduralMeshGenerator::create_mesh(MeshPtr mesh)
{
	...
	// so we can calculate it's memory usage right after creation
...
{
	// Check that the object is still valid, since this callback
	// might be called after the world is already unloaded.
	if (!obj)
		return;
...
{
	// Reset counters and memory statistics
...
void ProceduralMeshGenerator::update_stats()
{
	...
	// File and Blob modes use memory only when objects are active; otherwise they are cached
