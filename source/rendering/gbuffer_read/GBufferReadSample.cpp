// Sample UI for G-buffer extraction. Displays extracted textures on screen
// objects and supports switching between main camera and custom orbiting
// camera for G-buffer capture.
...
// Display nodes are connected to extracted textures; UI buttons toggle between camera modes.
...
// Orbiting camera transforms are calculated; node is rendered when in custom camera mode.
...
// Original visualizer state is restored and UI is cleaned up.
...
// Configures display node to show extracted texture via emission channel (unlit).
