// Modifies G-buffer content after deferred pass using MRT (Multiple Render
// Targets). Renders to temporary textures, applies color/plastic effects via
// custom shader, then swaps back into the G-buffer for lighting.
...
// G-buffer textures are modified in-place using temporary textures and a custom shader.
void GBufferWrite::g_buffers_ready_callback()
{
	...
	// so it's better to use this method than create new textures,
	// but we need to release these textures explicitly ourselves
	...
	// Set textures with flag BIND_FRAGMENT, so these textures can be usable only from fragment
	...
	render_target->enable();
	{
		...
		// to find out how you can use these parameters in shader)
		shader->bind();
		...
		// Here we bind textures to shader so we can read from them.
		// These lines correspond to shader lines
...
// G-buffer modification callback is registered; UI labels are created for displaying parameters.
...
// Effect parameters (color, plastic, influence) are animated over time for visual demonstration.
