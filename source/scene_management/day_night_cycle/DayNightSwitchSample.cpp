...
// Sample UI for the day/night cycle system. Provides sliders for time control,
// zenith angle threshold, morning/evening bounds, and timescale. Uses Visualizer
// to draw debug vectors showing sun direction versus vertical axis.
REGISTER_COMPONENT(DayNightSwitchSample);

// UI widgets are created and connected to SunController and DayNightSwitcher.
...
// Visualizer is disabled and UI window is released.
...
// Debug vectors are drawn: vertical axis (red) and sun direction (blue).
...
// Time in minutes is converted to "HH:MM" format string.
