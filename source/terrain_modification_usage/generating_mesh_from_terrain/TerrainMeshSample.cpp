...
// Visualizer is enabled; terrain mesh component is obtained; GUI is initialized.
...
// Status label is updated with current fetch progress.
...
// Visualizer is disabled; GUI resources are released.
...
// Transform is composed from GUI values and applied to terrain mesh node.
void TerrainMeshSample::update_terrain_mesh_parameters()
{
	// Transform is built from position, rotation, and scale
	Mat4 transform;
	...
	// Resolution and concurrency settings are applied
	terrain_mesh->setResolution(resolution);
	terrain_mesh->setMaxFetchers(concurrent_fetches);
}

// Sample window is created with parameter edit fields and controls.
void TerrainMeshSample::init_gui()
{
	...
	// Position X/Y edit fields are created
	...
	// Size X/Y edit fields are created
	...
	// Rotation angle edit field is created
	hbox = WidgetHBox::create(5);
	group_box->addChild(hbox, Gui::ALIGN_LEFT);
	...
	// Resolution X/Y edit fields are created
	hbox = WidgetHBox::create(5);
	group_box->addChild(hbox, Gui::ALIGN_LEFT);
	...
	// Concurrent fetches edit field is created
	hbox = WidgetHBox::create(5);
	group_box->addChild(hbox, Gui::ALIGN_LEFT);
	...
	// Visualization checkboxes are created
	...
	// Status label is created for progress display
	hbox = WidgetHBox::create(5);
	group_box->addChild(hbox, Gui::ALIGN_LEFT);
...
// Window widget is scheduled for deletion.
...
// =====================================================================
// GUI CALLBACKS - Update parameters when edit fields change
// =====================================================================
...
// Bounding box visualization is toggled.
